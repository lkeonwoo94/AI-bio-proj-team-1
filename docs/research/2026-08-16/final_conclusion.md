...
3. **암종별 성능 편차의 원인을 좁혔지만 규명하지는 못했다.** LOLO
   ...
   Random Forest 로 검정을 확장한 결과도 같은 결론이었다 — 자세한 내용은
   [2026-08-19/additional_results.md](../2026-08-19/additional_results.md) 참조.
...
## 암종별 성능 편차 · CIN/LOH 이진화 손실 — 후속 검증은 08-19 문서로 이동

"보완해야 할 점" 4번(암종별 성능 편차)과 5번(CIN/LOH 이진화 손실)에
대한 §26⑤·§26① 의 초기 진단은 위에서 다뤘다. 이를 더 깊이 파고든 후속
실험(Random Forest 로 확장한 가설 재검정, 회귀 기반 이진화 손실 직접
검증)은 문서 길이 때문에
[2026-08-19/additional_results.md](../2026-08-19/additional_results.md)
로 옮겼다 — 결론 요약은
[2026-08-19/final_conclusion.md](../2026-08-19/final_conclusion.md) 참고.
...
2. 선행 연구들은 개별 유전자 컬럼이 아니라 **더 뭉뚱그린 단위**를 입력으로
   ...
   구체적으로는 두 방향이 있었는데 실행 가능성이 달랐다.
   * **Pathway 단위 mutation burden** — 지금 가진 이진 행렬(유전자×세포주)
     그대로, 외부 gene-set 주석(예: MSigDB, DNA repair/cell cycle
     카테고리)만 있으면 바로 만들 수 있는 재집계였다.
   * **Mutation signature(trinucleotide context)** — 본 연구가 받은
     `OmicsSomaticMutationsMatrix{Hotspot,Damaging}.csv` 는 이미
     유전자×세포주 이진 행렬로 집계된 파일이라 염색체·위치·염기
     (Chrom/Pos/Ref/Alt) 정보가 사라져 있어, DepMap 이 별도로 배포하는
     원시 MAF 파일을 추가로 받아야 하는 확장 과제였다.

   두 방향과 독립 코호트(TCGA) 검증까지 세 실험 모두 실행했다 — 결과는
   [2026-08-19/additional_results.md](../2026-08-19/additional_results.md)
   (Pathway 는 음성, Mutation signature 는 양성, TCGA 는 하한 추정치),
   요약은
   [2026-08-19/final_conclusion.md](../2026-08-19/final_conclusion.md)
   참고.
