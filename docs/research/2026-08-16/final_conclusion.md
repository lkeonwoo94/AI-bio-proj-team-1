...
맞힐 확률이 10번 중 7번 정도라는 뜻이다. 연구용 신호로는 의미가 있으나
진단 목적으로는 부족하다.
...
**왜 특정 암종에서 무너지는가.** lineage 마다 WGD+ 기저율이 크게 다르다
(전체 65.2%).
...
"이 변이 조합은 WGD+ 경향" 규칙이 거의 작동하지 않는다. Eye 는 표본
22개로 추정 자체가 불안정하다.
...
CIN 6 종, LOH 3 종에서 무작위 수준(0.5) 미만이었다. 이는 lineage 별로
표현형 기저율이 크게 다르기 때문으로, 예컨대 WGD+ 비율은 전체 65.2% 인
반면 Lung 89.3%, Pancreas 91.3%, Bladder 94.1% 였다. 따라서 본 연구에서
도출된 변이 신호는 범암종에서 동일하게 작동하는 biomarker 라기보다
**lineage 의존적 후보**로 해석하는 것이 타당하다.
...
* 적용 범위: DepMap 세포주 코호트 내부 검증 결과이며, 암종 간 일반화는
  확인되지 않음
* 성능 수준: 연구용 신호 탐색에 적합하나 진단 목적에는 불충분
...
1. **성능 수준이 실용 임계 아래다.** 최고 ROC-AUC 0.765 는 mutation-only
   접근의 상한을 보여주는 값이지 진단 성능이 아니다.
2. **패널 안정성이 기준 미달이다.** README §18 의 네 기준 중 '안정성' 을
   충족하지 못했으므로 단일 고정 패널을 주장하지 않았다.
3. **lineage 기저율 차이를 통제하지 않았다.** LOLO 성능 저하가 변이 신호의
   한계 때문인지 기저율 차이 때문인지 분리하지 못했다. lineage 별 기저율을
   맞춘 재추출 분석이 필요하다.
