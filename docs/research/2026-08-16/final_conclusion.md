...
lineage 특이적인 변이-표현형 관계 차이일 가능성이 남지만, 본 연구의
설계로는 확인할 수 없다. 후속 과제로 남긴다.
...
3. **암종별 성능 편차의 원인을 규명하지 못했다.** LOLO 성능은 암종에 따라
   0.314~0.882 로 갈렸으나, 기저율 차이(rho +0.037)와 표본 크기(rho +0.095)
   어느 쪽으로도 설명되지 않았다. lineage 특이적 변이-표현형 관계 차이가
   후보이지만 본 설계로는 검증할 수 없다. 암종 내부에서 학습·평가하는
   lineage-specific 모델이 후속 과제로 필요하다.
